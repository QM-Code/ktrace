...
void PrintHelp(std::string_view app_name,
    ...
    std::cout
        ...
        << "\n"
        << "Options:\n";
    shared::AppendCommonHelp(std::cout, true);
    shared::AppendCoreBackendHelp(std::cout);
    AppendBackendHelp(std::cout);
...
AppOptions ParseAppOptions(int argc,
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        const auto render_result =
            ConsumeRenderBackendOption(arg, i, argc, argv, opts.backend_render, opts.backend_render_explicit);
        if (render_result.consumed) {
            if (!render_result.error.empty()) {
                Fail(render_result.error, opts.app_name, extra_options);
            }
            continue;
        }

        const auto ui_result = ConsumeUiBackendOption(arg, i, argc, argv, opts.backend_ui, opts.backend_ui_explicit);
        if (ui_result.consumed) {
            if (!ui_result.error.empty()) {
                Fail(ui_result.error, opts.app_name, extra_options);
            }
            continue;
        }
        ...
        const auto client_runtime_result = ConsumeRuntimeOption(arg,
                                                                i,
                                                                argc,
                                                                argv,
                                                                opts.username,
                                                                opts.username_explicit,
                                                                opts.password,
                                                                opts.password_explicit,
                                                                opts.server,
                                                                opts.server_explicit);
        if (client_runtime_result.consumed) {
            if (!client_runtime_result.error.empty()) {
                Fail(client_runtime_result.error, opts.app_name, extra_options);
            }
            continue;
        }
