...
using karma::common::data::ContentMount;
using karma::common::data::ContentMountType;
using karma::common::data::DataPathSpec;
...
void TraceMountTable(const std::optional<ContentMount> &filesystemMount,
                     ...
                     const char *reason) {
    if (!karma::common::logging::ShouldTraceChannel("content.mount")) {
        return;
    }
...
std::optional<karma::common::serialization::Value> LoadJsonFile(const std::filesystem::path &path,
...
std::filesystem::path ResolveConfiguredAsset(const std::string &assetKey,
    ...
    if (karma::common::config::ConfigStore::Initialized()) {
        const auto resolved = karma::common::config::ConfigStore::ResolveAssetPath(assetKey, defaultPath);
