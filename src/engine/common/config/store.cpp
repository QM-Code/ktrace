...
struct ConfigStoreState {
    ...
    uint64_t revision = 0;
    std::vector<karma::common::config::ConfigLayer> defaultLayers;
    std::optional<karma::common::config::ConfigLayer> userLayer;
    std::vector<karma::common::config::ConfigLayer> runtimeLayers;
    karma::common::serialization::Value defaults = karma::common::serialization::Object();
    karma::common::serialization::Value user = karma::common::serialization::Object();
    karma::common::serialization::Value merged = karma::common::serialization::Object();
...
const karma::common::serialization::Value *resolvePath(const karma::common::serialization::Value &root, std::string_view path) {
    if (path.empty()) {
        return &root;
    }

    const karma::common::serialization::Value *current = &root;
...
void mergeJsonObjects(karma::common::serialization::Value &destination, const karma::common::serialization::Value &source) {
...
void roundFloatValues(karma::common::serialization::Value &node) {
    if (node.is_object()) {
...
double readIntervalSeconds(const karma::common::serialization::Value &root, std::string_view path, double fallback) {
...
void collectAssetEntries(const karma::common::serialization::Value &node,
...
std::unordered_map<std::string, std::filesystem::path> buildAssetLookup(
    const std::vector<karma::common::config::ConfigLayer> &layers) {
...
std::vector<karma::common::config::ConfigLayer> loadLayers(const std::vector<karma::common::config::ConfigFileSpec> &specs) {
    std::vector<karma::common::config::ConfigLayer> layers;
    layers.reserve(specs.size());
    for (const auto &spec : specs) {
        std::filesystem::path path = spec.path;
        if (spec.resolveRelativeToDataRoot && path.is_relative()) {
            path = karma::common::data::Resolve(path);
        }
        ...
        auto jsonOpt = karma::common::data::LoadJsonFile(path, label, spec.missingLevel);
        if (!jsonOpt) {
            if (spec.required) {
...
void ConfigStore::Initialize(const std::vector<ConfigFileSpec> &defaultSpecs,
    ...
    karma::common::serialization::Value userJson = karma::common::serialization::Object();
    if (userConfigPath.has_value() && !userConfigPath->empty()) {
        ...
        if (auto userOpt = karma::common::data::LoadJsonFile(resolvedUserPath, "user config", spdlog::level::debug)) {
    ...
    karma::common::serialization::Value defaultsMerged = karma::common::serialization::Object();
...
const karma::common::serialization::Value &ConfigStore::Defaults() {
    std::lock_guard<std::mutex> lock(g_state.mutex);
    return g_state.defaults;
}

const karma::common::serialization::Value &ConfigStore::User() {
    std::lock_guard<std::mutex> lock(g_state.mutex);
    return g_state.user;
}

const karma::common::serialization::Value &ConfigStore::Merged() {
    std::lock_guard<std::mutex> lock(g_state.mutex);
    return g_state.merged;
}

const karma::common::serialization::Value *ConfigStore::Get(std::string_view path) {
    std::lock_guard<std::mutex> lock(g_state.mutex);
...
std::optional<karma::common::serialization::Value> ConfigStore::GetCopy(std::string_view path) {
    if (const auto *value = Get(path)) {
        return std::optional<karma::common::serialization::Value>(std::in_place, *value);
    }
    return std::nullopt;
}

bool ConfigStore::Set(std::string_view path, karma::common::serialization::Value value) {
    std::lock_guard<std::mutex> lock(g_state.mutex);
    if (!g_state.initialized) {
        return false;
    }
...
bool ConfigStore::ReplaceUserConfig(karma::common::serialization::Value userConfig, std::string *error) {
    if (!userConfig.is_object()) {
        userConfig = karma::common::serialization::Object();
    }
    std::lock_guard<std::mutex> lock(g_state.mutex);
    if (!g_state.initialized) {
        return false;
...
bool ConfigStore::saveUserUnlocked(std::string *error, bool ignoreInterval) {
    ...
    try {
        karma::common::serialization::Value rounded = g_state.user;
...
const karma::common::serialization::Value *ConfigStore::LayerByLabel(const std::string &label) {
    std::lock_guard<std::mutex> lock(g_state.mutex);
...
bool ConfigStore::setValueAtPath(karma::common::serialization::Value &root, std::string_view path, karma::common::serialization::Value value) {
    std::vector<std::pair<std::string, std::optional<std::size_t>>> segments;
    if (!parsePathSegments(path, segments)) {
        return false;
    }
    if (!root.is_object()) {
        root = karma::common::serialization::Object();
    }
    karma::common::serialization::Value *current = &root;
    for (std::size_t i = 0; i < segments.size(); ++i) {
        const auto &[key, index] = segments[i];
        const bool last = (i == segments.size() - 1);
        if (!key.empty()) {
            if (!current->is_object()) {
                *current = karma::common::serialization::Object();
            }
            ...
            if (!current->contains(key)) {
                (*current)[key] = index.has_value() ? karma::common::serialization::Array() : karma::common::serialization::Object();
            }
            current = &(*current)[key];
        }
        ...
        if (index.has_value()) {
            if (!current->is_array()) {
                *current = karma::common::serialization::Array();
            }
...
bool ConfigStore::eraseValueAtPath(karma::common::serialization::Value &root, std::string_view path) {
    std::vector<std::pair<std::string, std::optional<std::size_t>>> segments;
    if (!parsePathSegments(path, segments)) {
        return false;
    }
