...
| `jolt.md` | `overseer` | `in progress (new follow-on for pre-existing physics_backend_parity_jolt failure)` | Dispatch bounded fix slice for parity test asset-path resolver and re-run backend gates. | `2026-02-21` |
| `top-cmake.md` | `overseer` | `in progress (scope and structure contract drafted)` | Publish `TC0` specialist packet for top-level `CMakeLists.txt` decomposition with locked file layout/order. | `2026-02-21` |
...
| `multiplatform.md` | `overseer` | `in progress (policy baseline drafted; Linux local unblocker validated)` | `shared unblocker`: lock `MP0` linkage contract and dispatch `MP1` Linux runtime packaging hardening with installed-SDK smoke gate. | `2026-02-21` |
