...
CONFIGS = {
    "ktrace": ("KTraceSDK", "KTraceSDK", "KTraceSDKConfig.cmake"),
    "alpha": ("AlphaSDK", "AlphaSDK", "AlphaSDKConfig.cmake"),
    "beta": ("BetaSDK", "BetaSDK", "BetaSDKConfig.cmake"),
    "delta": ("DeltaSDK", "DeltaSDK", "DeltaSDKConfig.cmake"),
}
...
def usage(exit_code: int = 1) -> None:
    ...
    print("  --ktrace-sdk <dir>   required KTraceSDK prefix", file=sys.stderr)
    print("  --alpha-sdk <dir>    required AlphaSDK prefix", file=sys.stderr)
    print("  --beta-sdk <dir>     required BetaSDK prefix", file=sys.stderr)
    print("  --delta-sdk <dir>    required DeltaSDK prefix", file=sys.stderr)
...
def validate_sdk_prefix(prefix: str, sdk_key: str) -> None:
    pkg_name, cmake_dir, config_file = CONFIGS[sdk_key]
    config = os.path.join(prefix, "lib", "cmake", cmake_dir, config_file)
    if os.path.isfile(config):
        return
    print(
        f"Error: invalid --{sdk_key}-sdk prefix; missing package config for {pkg_name}.\n"
        f"Expected:\n  {config}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    while i < len(args):
        ...
        if arg in ("-h", "--help"):
            ...
            i += 1
            if i >= len(args):
                fail("missing value for --alpha-sdk")
            alpha_sdk_arg = args[i]
        elif arg == "--beta-sdk":
            i += 1
            if i >= len(args):
                fail("missing value for --beta-sdk")
            beta_sdk_arg = args[i]
        elif arg == "--delta-sdk":
            i += 1
            if i >= len(args):
                fail("missing value for --delta-sdk")
            delta_sdk_arg = args[i]
    ...
    if not alpha_sdk_arg:
        fail("--alpha-sdk <path> is required")
    if not beta_sdk_arg:
        fail("--beta-sdk <path> is required")
    if not delta_sdk_arg:
        fail("--delta-sdk <path> is required")
    ...
    alpha_sdk_prefix = resolve_prefix(alpha_sdk_arg, repo_root)
    beta_sdk_prefix = resolve_prefix(beta_sdk_arg, repo_root)
    delta_sdk_prefix = resolve_prefix(delta_sdk_arg, repo_root)

    validate_sdk_prefix(ktrace_sdk_prefix, "ktrace")
    validate_sdk_prefix(alpha_sdk_prefix, "alpha")
    validate_sdk_prefix(beta_sdk_prefix, "beta")
    validate_sdk_prefix(delta_sdk_prefix, "delta")
    ...
    prefix_path = ";".join(
        [ktrace_sdk_prefix, alpha_sdk_prefix, beta_sdk_prefix, delta_sdk_prefix, vcpkg_prefix]
    )
    ...
    cmake_args = [
        ...
        f"-DAlphaSDK_DIR={os.path.join(alpha_sdk_prefix, 'lib', 'cmake', 'AlphaSDK')}",
        f"-DBetaSDK_DIR={os.path.join(beta_sdk_prefix, 'lib', 'cmake', 'BetaSDK')}",
        f"-DDeltaSDK_DIR={os.path.join(delta_sdk_prefix, 'lib', 'cmake', 'DeltaSDK')}",
    ...
    if no_configure:
        ...
        run(["cmake", "-S", script_dir, "-B", build_dir, *cmake_args], env=env)
