...
| `renderer-parity.md` | `specialist-renderer-parity` | `priority/in progress (R26-A complete; R26-B slice 1 landed; R26-B slice 2 scaffolding landed; R26-B slice 3 BGFX GPU prototype landed; R26-B slice 4 Diligent GPU prototype landed; R26-B slice 5 lifecycle/fallback hardening landed; R26-B visual closeout checkpoint captured with Diligent screenshot tooling blocker documented; R26-B gpu_default no-shadow fix landed pending operator visual verify; R26-C intake matrix landed)` | Operator-verify `gpu_default` shadow restoration on BGFX + Diligent (visual + trace capture), then execute `R26-D` (`shared unblocker`) config policy expansion. | `2026-02-14` |
