...
## Project Snapshot
- Current owner: `codex` (integration-owner role)
- Status: `in progress`
- Immediate next task: drive renderer capability integration as the top milestone while keeping cross-project contracts stable.
- Validation gate: run project-specific gates from touched project docs; for mixed-scope changes run both `./scripts/test-engine-backends.sh` and `./scripts/test-server-net.sh`.

## Project Linkage
- Delegation index: `docs/projects/README.md`
- Delegation rules: `docs/AGENTS.md`

## Purpose
This document tracks core engine infrastructure progress and the next steps required before broad parallel delegation to other agents.

Primary target:
- Keep engine lifecycle ownership solid while completing backend-pluggable core subsystems.
- Provide stable engine contracts so gameplay/UI/network ports can proceed without architecture drift.

Non-goals:
- Porting gameplay behavior (shots, damage, respawn, score rules, etc).
- Recreating `m-dev` structure exactly.
...
## Completed
1. Physics backend infrastructure skeleton
- Engine-owned physics interfaces and lifecycle hooks exist.
- Backend contract supports `auto|jolt|physx`.
- Backend factory + startup traces are integrated.
- Client/server CLI and config can override physics backend.

2. Audio backend infrastructure skeleton
- Engine-owned audio interfaces and lifecycle hooks exist.
- Backend contract supports `auto|sdl3audio|miniaudio`.
- Backend factory + startup traces are integrated.
- Client/server CLI and config can override audio backend.

3. Fixed-step simulation clock
- Accumulator-based simulation clock is implemented and wired into client/server engine loops.
- Physics uses fixed-step simulation path.
- Base `engine.sim` channel is high-signal; per-frame diagnostics moved to `engine.sim.frames`.

4. Backend capability/selection reporting
- Startup traces report requested/compiled/selected backend for render/physics/audio.
- Invalid backend selections fail clearly in parsing/validation paths.
...
- Implementation-ready system scheduling contract is now documented in this file (`phase enum`, registration schema, dependency rules, cycle-fail behavior, override boundaries).
- First-slice acceptance criteria and required tests are explicit to support direct implementation handoff.

7. Component catalog contract first-slice spec is codified
- Implementation-ready component catalog contract is now documented in this file (canonical metadata schema, ownership/source-of-truth rules, serialization/versioning policy, validation/failure behavior, override boundaries).
- First-slice acceptance criteria and required tests are explicit to support direct implementation handoff.

8. Physics API layering contract first-slice spec is codified
- Implementation-ready physics API layering contract is now documented in this file (core `BodyId`/system boundaries, optional 95%-path facade boundaries, override/extension limits, validation/failure behavior).
- First-slice acceptance criteria and required tests are explicit to support direct implementation handoff.

9. Audio API layering contract first-slice spec is codified
- Implementation-ready audio API layering contract is now documented in this file (core deterministic request/voice boundaries, optional 95%-path facade boundaries, override/extension limits, validation/failure behavior).
- First-slice acceptance criteria and required tests are explicit to support direct implementation handoff.

10. Stable-first implementation sequencing is codified
- A contract-focused execution order across scheduler/component/physics/audio stages is now documented with explicit stage entry/exit criteria, test expectations, wrapper-gate closeout rules, and parallelization constraints.

11. Stage-3 physics-core kickoff packet is codified
- A dedicated Stage-3 kickoff packet now defines entry checks, test-first execution order, exit criteria, wrapper closeout reminders, and scope boundaries that block scheduler/component/audio creep before physics core exit.

12. Stage-4 physics-facade/override kickoff packet is codified
- A dedicated Stage-4 kickoff packet now defines entry checks, test-first execution order, exit criteria, wrapper closeout reminders, and scope boundaries that block scheduler/component/audio/core-physics creep before physics facade/override exit.

13. Stage-5 audio-core kickoff packet is codified
- A dedicated Stage-5 kickoff packet now defines entry checks, test-first execution order, exit criteria, wrapper closeout reminders, and scope boundaries that block scheduler/component/physics (including Stage-3 and Stage-4 physics scopes) creep before audio-core exit.

14. Stage-6 audio-facade/override kickoff packet is codified
- A dedicated Stage-6 kickoff packet now defines entry checks, test-first execution order, exit criteria, wrapper closeout reminders, and scope boundaries that block scheduler/component/physics (including Stage-3 and Stage-4 physics scopes) and Stage-5 audio-core creep before audio-facade/override exit.

15. Consolidated stages 1-6 handoff checklist is codified
- A single implementation handoff checklist now maps each stage to an execution owner role and required closeout evidence so agent handoffs are consistent and reviewable.
...
5. Harden backend behavior consistency and trace ergonomics across matrix combinations.
