...
## Purpose
This file is the canonical execution policy for delegated work (to coding agent specialists):
...
- Standalone mode: if the current directory is the `m-karma` repository root (contains `abuild.py` and `docs/`), use unprefixed repo-relative paths (`docs/AGENTS.md`, `docs/...`).
...
- If staying at integration workspace root, prefix every project path with `m-karma/`.
...
## KARMA -> BZ3 SDK Contract (Required)
This contract is locked and must be treated as stable unless explicitly changed in cross-repo policy/docs.
- Official integration path: `m-bz3` consumes KARMA via `find_package(KarmaEngine CONFIG REQUIRED)`.
- Direct raw include/lib wiring from `m-bz3` into KARMA build artifacts is not allowed.
- SDK producer command (`m-karma`):
  - `./abuild.py -c -d build-sdk --install-sdk out/karma-sdk`
- SDK consumer command (`m-bz3`):
...
- Required seam invariants:
  ...
  - backend headers/types stay out of non-backend modules and public contracts,
...
- Touching `src/network/*` or `include/karma/network/*`:

```bash
./scripts/test-server-net.sh <build-dir>
```

- Touching `src/physics/*`, `src/audio/*`, `include/karma/physics/*`, `include/karma/audio/*`, or backend-test wiring in `src/engine/CMakeLists.txt`:
